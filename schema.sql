...
create table if not exists public.referrals (
  ...
  commission_rate_percent numeric(5,2) not null default 10.00 check (commission_rate_percent >= 0 and commission_rate_percent <= 50),
  commission_daily_cap integer not null default 20 check (commission_daily_cap >= 0 and commission_daily_cap <= 500),
...
create table if not exists public.referral_activity_commissions (
  ...
  inviter_id uuid references public.profiles(id) on delete cascade not null,
  invitee_id uuid references public.profiles(id) on delete cascade not null,
  source_transaction_id uuid references public.transactions(id) on delete cascade not null unique,
...
create index if not exists votes_voter_target_created_idx on public.votes (voter_id, target_id, created_at desc);
...
create index if not exists referrals_invitee_commission_idx on public.referrals (invitee_id, status, commission_expires_at);
create index if not exists referral_activity_commissions_inviter_created_idx on public.referral_activity_commissions (inviter_id, created_at desc);
create index if not exists referral_activity_commissions_invitee_created_idx on public.referral_activity_commissions (invitee_id, created_at desc);
...
begin
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'referral_activity_commissions' and policyname = 'referral_activity_commissions_select_related') then
    create policy referral_activity_commissions_select_related on public.referral_activity_commissions for select using (auth.uid() = inviter_id or auth.uid() = invitee_id);
  end if;
