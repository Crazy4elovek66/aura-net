...
function getReferralStage(referral: ReferralEntry) {
  if (referral.status === "activated") {
    return {
      key: "activated",
      label: "Активирован",
      detail: `Готово: ты получил +${referral.inviterReward}, друг получил +${referral.inviteeReward}.`,
  ...
  if (referral.hasFirstClaim) {
    return {
      ...
      label: "Ждём действие",
      detail: "Первый вход уже был. Нужен любой голос по карточке, чтобы закрыть активацию.",
  ...
  return {
    ...
    detail: "Человек привязался по ссылке. Следующий шаг: забрать первую награду дня.",
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/70">Инвайты</h2>
          <p className="mt-2 text-[11px] leading-relaxed text-white/55">
            Здесь видно, кто уже пришел по ссылке, кто застрял и кого нужно дожать до активации.
            После активации ты получаешь процент с их плюс-голосов.
          </p>
        </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3">
          <p className="text-[10px] uppercase tracking-[0.1em] text-white/45">Твой код</p>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3">
          <p className="text-[10px] uppercase tracking-[0.1em] text-white/45">Доход после активации</p>
          <p className="mt-2 text-[11px] leading-relaxed text-white/65">
            Ты получаешь {REFERRAL_ACTIVITY_PERCENT}% с плюс-голосов приглашенного.
            Лимит: до +{REFERRAL_ACTIVITY_DAILY_CAP} в сутки с одного человека, окно выплат: {REFERRAL_ACTIVITY_DURATION_DAYS} дней.
          </p>
        </div>
      </div>
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-4">
        <div className="rounded-2xl border border-white/10 bg-black/20 p-3">
          <p className="text-[9px] uppercase tracking-[0.1em] text-white/45">По ссылке</p>
        ...
        <div className="rounded-2xl border border-neon-purple/20 bg-black/20 p-3">
          <p className="text-[9px] uppercase tracking-[0.1em] text-white/45">Без входа</p>
        ...
        <div className="rounded-2xl border border-neon-pink/20 bg-black/20 p-3">
          <p className="text-[9px] uppercase tracking-[0.1em] text-white/45">Без действия</p>
      ...
      <div className="mt-4 space-y-2">
        ...
        >
          <span className="truncate pr-3">{webInviteLink || "Ссылка пока недоступна"}</span>
        ...
        {sentState ? (
          <p className="rounded-2xl border border-neon-green/20 bg-neon-green/[0.06] px-3 py-2 text-[10px] uppercase tracking-[0.08em] text-neon-green/90">
            Ссылка скопирована. Как только человек зайдет, он появится ниже.
          </p>
        ) : null}
      ...
      <div className="mt-4 space-y-2">
        {referrals.length ? (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-[10px] uppercase tracking-[0.08em] text-white/45">
                      Зашел по ссылке: {formatDate(referral.joinedAt)} UTC+0
                    </p>
                  </div>
          ...
          <p className="rounded-2xl border border-white/10 bg-white/[0.02] px-3 py-3 text-[11px] leading-relaxed text-white/55">
            Пока никого нет. Рабочий старт: сначала коротко объясни, зачем Aura, потом сразу отправляй ссылку.
          </p>
        )}
      </div>
    </section>
